...
### P3 — ГОТОВО (см. выше); открыты отложенные пункты TD-17.4/TD-18.1/4:
untrack `commit-push.bat` (коллабораторы), судьба CSV-пайплайна
`scripts/build-/import-directus-products-csv.mjs` (архив или `scripts/lib/`),
архивация one-off'ов `sync-deere-shop-brand.mjs`/`upload-deere-shop-logo.mjs`,
`git rm -r --cached` одноразовых outputs, рассинхрон `.gitignore`
(`outputs/`, `docs/superpowers/`). Всё — явные «да» владельца.
...
- [ ] Решения владельца: пуш/релиз P3; отложенные пункты TD-17.4/TD-18.1/4
      (см. выше); остатки на VPS (косметика); импорт данных аналогов;
