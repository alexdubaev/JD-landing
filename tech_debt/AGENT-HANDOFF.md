...
### Решения владельца по остаткам гигиены приняты 2026-08-28 и выполнены:
- one-off'ы sync-brand/upload-logo архивированы (`197ddf6`);
- 1257 файлов одноразового импорта выведены из git (`681e10e`), ignore
  outputs/ теперь консистентен (в git их нет, файлы остались на диске и в
  истории; на LFS-объекты rm --cached не влияет);
- рассинхрон docs/superpowers устранён: все plans/specs затреканы,
  ignore-строка удалена (файлы, которые она прятала, — `git add`).
...
- [ ] Открытые темы владельца: остатки на VPS (косметика), импорт данных
      аналогов, stage-2 S-1…S-12 (обсуждение). По гигиене репо всё закрыто.
