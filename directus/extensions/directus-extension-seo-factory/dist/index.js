...
export const claimApproved = ({ database, accountability, request }) => database.transaction(async (trx) => {
  ...
  const rows = await trx("seo_work_items").whereIn("status", CLAIMABLE).andWhere((query) => query.whereNull("expires_at").orWhere("expires_at", "<=", trx.fn.now())).orderBy("created_at", "asc").forUpdate().skipLocked().limit(boundedLimit(request.body?.limit));
  const claimed = [];
  for (const row of rows) {
    await trx("seo_factory_claims").insert({ work_item_id: row.id, run_id: runId(request), state: "processing", lease_until: leaseUntil, attempts: 1, updated_at: now }).onConflict("work_item_id").merge({ run_id: runId(request), state: "processing", lease_until: leaseUntil, attempts: trx.raw("?? + 1", ["attempts"]), updated_at: now, last_error: null });
...
function handler(action, context) {
  return async (request, response) => {
    try { response.json({ data: await action({ ...context, accountability: request.accountability }, { ...request, context }) }); }
    catch (error) { response.status(error.code === "FORBIDDEN" ? 403 : 500).json({ error: error.code === "FORBIDDEN" ? "forbidden" : "claim_failed" }); }
  };
}
